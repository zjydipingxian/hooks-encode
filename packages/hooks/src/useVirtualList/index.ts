...
const useVirtualList = <T = any>(state: T[], options: OptionType) => {
  ...
  const calculateRange = () => {
    ...
    if (element) {
      ...
      const startIndex = Math.max(offset - overscan, 0);
      const endIndex = Math.min(offset + viewCapacity + overscan, state.length);

      // 更新列表
      list.value.splice(0, list.value.length, ...state.slice(startIndex, endIndex));

      // 实时计算
      offsetTop = getDistanceTop(startIndex);
