...
export async function updateCreateRouterFile(hooksType, folderName) {
  const spinner = ora().start(`docs目录下，正在更新 generate-router.js 路由文件...`);
    ...
    traverse(ast, {
      ObjectExpression(path) {
        ...
        if (targetProp) {
          ...
          // 如果不存在，则添加新属性
          path.node.properties.push(t.objectProperty(t.identifier(hooksType), t.arrayExpression([t.stringLiteral(folderName)])));
        }
      },
    });
  ...
  spinner.succeed(`docs目录下，${chalk.green(`generate-router.js `)}路由文件更新成功`);
}
